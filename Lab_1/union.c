...
int cal_union(long int set1, long int set2, long int *result)
{
  *result = set1 | set2;
...
int main(int argc, char *argv[])
{
  ...
  long int union_set = 0L;
  int size = cal_union(set1, set2, &union_set);

  FILE *out = fopen("output.txt", "w");

  // Write the set to output file
  write_set(out, union_set);

  // Print size of union set to console
  printf("Size of union set: %d\n", size);
